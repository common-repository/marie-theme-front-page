<?php
/**
 * Widget to show the fixed address & phone.
 *
 * @package marie-tfp
 * @version 1.0.0
 */
...
class Address extends WP_Widget {
	...
	/**
	 * AddressWidget class constructor.
	 */
	public function __construct() {
		$this->widget_id       = 'marie_tfp_address_widget';
		$this->widget_name     = 'Address Widget';
	...
	public function widget( $args, $instance ) {
		...
		//phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		echo $this->render_address_and_phone();
	...
	public function update( $updated_instance, $old_instance ) {
		...
		/**
		 * Address and Phone in footer widget.
		 */
	public function render_address_and_phone() {

		// Company Information.
		$company_address_1 = get_theme_mod( 'marie_company_settings_address_1' );
		$company_phone     = get_theme_mod( 'marie_company_settings_company_phone' );

		// Don't do anything if no address/phone set.
		...
		<?php
		if ( ! empty( esc_html( $company_address_1 ) ) ) :
			?>
				<span class="<?php echo ! empty( esc_html( $company_phone ) ) ? 'marie-pipe' : ''; ?>">
					<?php echo esc_html( $company_address_1 ); ?>
				</span>
			<?php
			endif;
		if ( ! empty( esc_html( $company_phone ) ) ) :
			?>
